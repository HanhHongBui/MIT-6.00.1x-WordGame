...
def getWordScore(word, n):
	...
	for letter in word:
		...
		sum_of_score += letter_score
	total_score = len(word)*sum_of_score
	if len(word) == n:
		total_score += 50
	return total_score	
...
def isValidWord(word, hand, wordList):
	...
	if not word in wordList:
		...
		# check if hand contains all the letters of word
		for letter in word:
			if not letter in hand_keys:
				isvalid = False
			else:
				#compare the frequency of letter
...
def playGame(wordList):
	...
	while True:
		...
		# r: replay the last hand
			...
			# first time => warning if storedHand is empty
			if not storedHand:
				print("You have not played a hand yet. Please play a new hand first!")
			#playhand on previous hand
			else:
